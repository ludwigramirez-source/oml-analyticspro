...
class ConfigService:
    ...
    async def create_config(self, config_data: DatabaseConfigCreate) -> DatabaseConfig:
        ...
        if test_result.success:
            try:
                ...
                logger.info("Global connection updated successfully")
            except Exception as e:
                logger.warning(f"Warning: Could not update global connection: {e}")

        logger.info(f"Configuration saved: {config.db_host}:{config.db_port}/{config.db_name}")
    ...
    async def activate_config(self, config_id: str) -> bool:
        ...
        if result.modified_count > 0:
            logger.info(f"Configuration activated: {config_id}")
            ...
            try:
                ...
                if config:
                    ...
                    connection_string = self.get_connection_string(config)
                    update_database_connection(connection_string)
                    logger.info(f"Global connection updated to: {config.db_host}:{config.db_port}/{config.db_name}")
            except Exception as e:
                logger.error(f"Error updating global connection: {e}")
