...
const TablaAbandonadasOptimizada = ({ llamadasPaginated, onPageChange, filters }) => {
  ...
  const handleExportVisible = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Llamadas Abandonadas');
    XLSX.writeFile(wb, `llamadas_abandonadas_pagina_${currentPage}_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
      ...
      <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
        <div>
          <h3 className="text-lg font-semibold text-gray-800">
            Llamadas No Atendidas / Abandonadas
          </h3>
                ...
                <tr key={idx} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-xs">
                    <span className={`px-2 py-1 rounded-full ${
                      llamada.evento === 'ABANDON' ? 'bg-red-100 text-red-800' :
                      llamada.evento === 'EXITWITHTIMEOUT' ? 'bg-orange-100 text-orange-800' :
...
export default TablaAbandonadasOptimizada;
